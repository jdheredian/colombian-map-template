"""
Tema para mapas a escala de pais. Colombia entera. Mas alta que ancha.
...
grosor = 0.08              # ancho de linea para esta escala
...
tema_pais = theme_void(base_size=base, base_family=fuente) + theme(
...
# import pandas as pd
# from plotnine import *
# from tema_pais import tema_pais, paletas, borde, grosor, vecino, vecino_borde
#
# # EPSG:9377 es la proyeccion oficial de Colombia. Sin reproyectar,
# # el pais sale comprimido en vertical.
...
#
# # San Andres (codigo 88) queda a 700 km: estira el mapa en oceano
# # vacio. Va en un inserto; ver la seccion 6 del README.
# df_continental = df_mpio.query("dpto_ccdgo != '88'")
#
# # Cuantiles y no intervalos iguales: casi toda variable
# # socioeconomica colombiana es muy asimetrica.
# cortador = mapclassify.Quantiles(df_continental["mi_variable"], k=5)
# cortes = np.concatenate([[df_continental["mi_variable"].min()],
#                          cortador.bins])
# etiquetas = [f"{cortes[i]:,.1f} – {cortes[i+1]:,.1f}"
#              .replace(",", "@").replace(".", ",").replace("@", ".")
#              for i in range(5)]
# posicion = np.digitize(df_continental["mi_variable"],
#                        cortador.bins[:-1], right=True)
# df_continental["clase"] = pd.Categorical(
